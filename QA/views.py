# QA/views.py - COMPLETE WORKING VERSION WITH LANGUAGE SUPPORT
...
def get_qa_language(request):
    """Get current QA language from session, update from ?lang= param."""
    if request.GET.get('lang') in ('en', 'hi'):
        request.session['qa_language'] = request.GET.get('lang')
    return request.session.get('qa_language', 'en')


def localize_subject(s, language):
    s.localized_name = s.get_name(language)
    s.localized_description = s.get_description(language)
    return s


def localize_topic(t, language):
    t.localized_name = t.get_name(language)
    t.localized_description = t.get_description(language)
    return t


def localize_part(p, language):
    p.localized_name = p.get_name(language)
    p.localized_description = p.get_description(language)
    return p


def localize_question(q, language):
    q.localized_question = q.get_question_text(language)
    q.localized_answer = q.get_answer_text(language)
    return q


def localize_category(c, language):
    c.localized_name = c.get_name(language)
    return c


def localize_comprehensive(c, language):
    c.localized_heading = c.get_heading(language)
    c.localized_content = c.get_content(language)
    return c
...
def subject_list(request):
    language = get_qa_language(request)
    ...
    for subject in subjects:
        localize_subject(subject, language)
...
def topic_list(request, subject_slug):
    ...
    if subject.is_locked() and not subject.user_has_access(request.user):
        messages.warning(request, f'"{subject.localized_name}" is locked. Please purchase to access.')
    ...
    for c in comprehensive_content:
        localize_comprehensive(c, language)
...
def part_list(request, subject_slug, topic_slug):
    ...
    if topic.is_locked() and not topic.user_has_access(request.user):
        messages.warning(request, f'"{topic.localized_name}" is locked. Please purchase to access.')
    ...
    if request.GET:
        ...
        filtered_questions_list = list(questions[:50])
        for q in filtered_questions_list:
            localize_question(q, language)
        filtered_questions = filtered_questions_list
...
def part_detail(request, subject_slug, topic_slug, part_slug):
    ...
    if part.is_locked() and not part.user_has_access(request.user):
        messages.warning(request, f'"{part.localized_name}" is locked. Please purchase to access.')
    ...
    for q in questions:
        localize_question(q, language)
        for cat in q.categories.all():
            localize_category(cat, language)
    ...
    # ✅ FIX: pass language
    rendered_content = part.render_content(language=language)
    ...
    if PAYMENTS_AVAILABLE:
        try:
            ContentAccessLog.objects.create(
                ...
                content_id=part.id,
                content_name=part.localized_name,
                content_app='qa',
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        context = {'questions': questions, 'language': language}
...
def topic_detail(request, slug):
    ...
    all_questions = Question.objects.filter(
        part__topic=topic, is_active=True
...
def get_topics_api(request, subject_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'subject_name': subject.get_name(language),
            'subject_slug': subject.slug,
            'topics': topics_data,
        })
...
def get_topics_by_subject_api(request, subject_id):
    ...
    try:
        ...
        topics_data = [
            {'id': t.id, 'name': t.get_name(language), 'slug': t.slug}
            for t in topics
        ]
...
def get_parts_by_topic_api(request, topic_id):
    ...
    try:
        ...
        parts = topic.parts.filter(is_active=True)
        parts_data = [
            {'id': p.id, 'name': p.get_name(language), 'slug': p.slug}
            for p in parts
        ]
